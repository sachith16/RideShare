function initMap(){
	...
	var displayC2 = new google.maps.DirectionsRenderer({preserveViewport: true});
	...
	displayC2.setOptions( { suppressMarkers: true,polylineOptions: {
	            strokeWeight: 10,
	            strokeOpacity: 1,
	            strokeColor:  'pink' 
	        }});
	...
	function matchPoints(){
		...
		servicesC2.route(request2,function(result,status){
			if(status =='OK'){
				displayC2.setDirections(result);								
			}
		});

		}else{
			displayC.setMap(null);
			displayC2.setMap(null);
