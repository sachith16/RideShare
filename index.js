...
//handling add ride to database POST request 
...
//handling add ride request to database POST request 
app.post('/add-ride-request', function(req, res) {
	session=req.session;
	user=session.username;	
	var ref = db.ref("requests/"+user);
	ref.set({username: user,
			deviceid: req.body.deviceid,
			origin_name: req.body.origin_name,
			destination_name: req.body.destination_name,
...
//handling checking if user already has a ride POST request 
...
//handling remove ride from database POST request 
...
//handling matching ride when user requests POST request 
	    ...
	    if(snapshot.exists()){
	    	snapshot.forEach((child) => {
				...
				if(matchedpoints.length>1){
					lastUsername=child.val().username;
					...
					getDistanceMatch(child.val().origin,child.val().destination,rqst.body.origin,
						rqst.body.destination,matchedpoints[0],matchedpoints[matchedpoints.length-1],function(result,duration,costS,costR,distanceM){
							...
							console.log(lastUsername+" "+child.val().username+" "+matchedDeviceID);
							if(lastUsername==child.val().username && matchedDeviceID!=''){
								...
								//res.end('No matching rides at the time');
							}
					});
				}	
			});	
...
//handling get all rides in database POST request 
...
//handling get all rides in database POST request 
app.post('/get-all-rides-requests', function(req, res) {
...
//handling details of an user POST request 
app.post('/get-user-details', function(req, res) {
	var ref = db.ref("users/"+req.body.username);
	ref.once("value",function(snapshot) {
	   	if(snapshot.exists()){
...
//handling respond to matched user POST request 
